...
class TypeTest:
            ...
            for bit_ in self.bit:
                ...
                for num in test_data[int_][bit_]:
                    if deploy_model == 'proxy':
                        ...
                        encode_data = self.wasm_obj.encodeABI(fn_name=f"set{int_}{bit_}", args=[num])
    ...
    def other_type(self, type_, param):
        ...
        if deploy_model == 'proxy':
            ...
            encode_data = self.wasm_obj.encodeABI(fn_name=f"set{type_}", args=[param])
            SendTxn(transaction_dict(encode_data, con_address))
...
# type_test = TypeTest(con_address, abi_info, type_='List')
# type_test.run()
# endregion
